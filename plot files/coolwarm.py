...
# to correct for round-off errors associated with the interpolation.
    map = np.array([np.transpose(r),np.transpose(g),np.transpose(b)]) / 255
    map = map / np.amax(map)
    return map