...
def pdf2eps(source = None,dest = None): 
    # Construct the options string for pdftops
    options = np.array(['-q -paper match -eps -level2 "',source,'" "',dest,'"'])
    ...
    if status:
        # Report error
        if len(message)==0:
            raise Exception('Unable to generate eps. Check destination directory is writable.')
        else:
    ...
    if str(str(np.arange(1,np.amin(13,end())+1))) == str('% Produced by'):
        fseek(fid,- np.asarray(str).size - 1,'cof')
        fwrite(fid,'%')
