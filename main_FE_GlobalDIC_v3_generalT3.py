...
scipy.io.loadmat('./Images_Sample12/mesh_plate_hole_T3.mat')
...
# ####################################################################
    ...
    if ImgSeqNum == 2 or DICpara.NewFFTSearch == 1:
        ...
        Plotdisp_showTri(U0,DICmesh.coordinatesFEM,DICmesh.elementsFEM)
...
# alphaList = [1e-3,1e-2,1e-1,1e0,1e1,1e2,1e3]*mean(DICpara.winstepsize);
    ...
    for alphaInd in np.arange(1,len(alphaList)+1).reshape(-1):
        ...
        U,normOfW,timeICGN = funGlobalICGNT3(DICmesh,Df,fNormalized,gNormalized,U0,alpha,DICpara.tol,DICpara.maxIter)
        Plotdisp_showTri(U,DICmesh.coordinatesFEM,DICmesh.elementsFEM)
        # Compute F deformation gradient with solved U
        F = funGlobal_NodalStrainT3(DICmesh.coordinatesFEM,DICmesh.elementsFEM,U)
    ...
    if np.abs(alpha_best - alpha) > np.abs(eps):
        U,normOfW,timeICGN = funGlobalICGNT3(DICmesh,Df,fNormalized,gNormalized,U0,alpha_best,DICpara.tol,DICpara.maxIter)
        F = funGlobal_NodalStrainT3(DICmesh.coordinatesFEM,DICmesh.elementsFEM,U)
    # ------- Smooth strain field --------
...
close_('all')
Plotdisp_showTri(UWorld,DICmesh.coordinatesFEMWorld,DICmesh.elementsFEM)
Plotstrain_showTri(FWorld,DICmesh.coordinatesFEMWorld,DICmesh.elementsFEM)
## ------ Save results ------
