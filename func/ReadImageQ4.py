...
# fprintf('Coordinates of bottom-right corner point are (#4.3f,#4.3f)\n',gridx(2), gridy(2))
    ...
    gridx = np.array([1,Img[0].shape[1-1]])
    gridy = np.array([1,Img[0].shape[2-1]])
    gridxy.gridx = np.round(gridx)
    gridxy.gridy = np.round(gridy)
    # Choose finite element size
    print('\n' % ())
    print('--- What is the initial guess window size (unit: px) ? --- \n' % ())
